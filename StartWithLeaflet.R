...
leaflet() %>%
  ...
  addMarkers(lng = 151.005006, lat = -33.9767231, popup = popup) 
...
l_dan <- leaflet() %>%   # assign the base location to an object
  setView(10.2055396,56.1725556, zoom = 20)
...
addMarkers(10.2055396,56.1725556, popup = popup) 
...
danishmonuments <- read_csv("data/CapturingCoordinates - SA2022.csv")
...
DANmap2 <- DANmap %>% 
  ...
  addMarkers(lng = places$Longitude, 
             ...
             popup = paste("Stednavn:", places$Description, "<br>",
                           "Beskrivelse:", places$Type, "<br>",
                           "Stjerner:", places$Stars1_5),
             clusterOptions = markerClusterOptions())

DANmap2

saveWidget(DANmap2, "DANmap.html", selfcontained = TRUE)
